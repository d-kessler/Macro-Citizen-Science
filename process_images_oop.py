...
class ProcessImages:
    ...
    def __init__(self, username, password, project_id, workflow_id, parent_folder="images",
        ...
        for subfolder in self.subfolders:
            ...
            for image_file_name in self.image_file_names:
                ...
                self.grain_density, self.grain_stats = self.get_grain_stats_and_density(self.exp_image_file_path)

                # TODO: quantify glare area
                ...
                self.exp_metadata_list = [self.exp_subject_id, str(image_file_name), self.warehouse_name, self.location,
                                          ...
                                          self.grain_density, self.grain_stats, self.number_of_columns]
            ...
            for image_file_name in self.image_file_names_sample_sims:
                ...
                self.sim_pil_img = self.draw_sim()
    ...
    def draw_sim(self):
        ...
        # TODO: avoid glare areas

        pass

        return self.sim_pil_img
    ...
    def get_date_exif(self):
        ...
        try:
            date = datetime.strptime(self.exif_dict['DateTimeOriginal'], '%Y:%m:%d %H:%M:%S')
    ...
    def get_grain_stats_and_density(self, image_file_path):
        ...
        # Detecting edges
        lower_canny, upper_canny = self.canny_thresholds(opened_img)
        ...
        for cont in contours_list:
            ...
            # Setting grain size equal to a tuple of the contour's height and width in millimeters
            grain_size = (mm_height, mm_width)
            grain_avg_dim = (mm_height + mm_width) / 2
            # Ignoring specs that have an average dimension less than 0.015 millimeters (decided via experimentation)
            if grain_avg_dim < 0.015:
        ...
        # Getting a list of grain areas in square pixels
        grain_areas = []
        for cont in grain_contours:
            area = cv2.contourArea(cont)
            grain_areas.append(area)
        # Finding the total grain area in square pixels
        ...
        # Getting statistics on the image's grain, returning for storage in the image's manifest entry
        h = [g[0] for g in grain_sizes]
        w = [g[1] for g in grain_sizes]
